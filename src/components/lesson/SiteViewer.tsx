    ...
    <div
      ...
      className={
        full
          ...
          : "rounded-3xl border-2 border-border bg-card overflow-hidden shadow-md"
      }
    >
      {/* Browser-like chrome: traffic lights, live address, device width, fullscreen. */}
      <div className="shrink-0 bg-foreground text-background">
        <div className="flex items-center gap-2 px-3 py-2">
          <span className="hidden sm:flex items-center gap-1" aria-hidden>
            <span className="w-2.5 h-2.5 rounded-full bg-heart/80" />
            <span className="w-2.5 h-2.5 rounded-full bg-streak/80" />
            <span className="w-2.5 h-2.5 rounded-full bg-primary/80" />
          </span>
          <Globe className="w-4 h-4 shrink-0" />
          <span className="text-[13px] font-extrabold truncate">{spec.title ?? "عارض المواقع"}</span>
          <div className="flex-1" />
          {!windowMode && (
            <>
              <button
                onClick={() => setDevice((d) => (d === "phone" ? "full" : "phone"))}
                aria-label="عرض كالجوال"
                className={`p-1 rounded-lg hover:bg-background/20 ${device === "phone" ? "bg-background/25" : ""}`}
              >
              ...
              <button onClick={() => setFull((f) => !f)} aria-label="ملء الشاشة" className="p-1 rounded-lg hover:bg-background/20">
        ...
        <div className="px-3 pb-2" dir="ltr">
          <div className="flex items-center gap-1.5 rounded-full bg-background/15 px-3 py-1">
            <Lock className="w-3 h-3 shrink-0 opacity-80" />
            <span className="text-[11px] font-bold truncate opacity-90">{currentUrl}</span>
          </div>
        </div>
      </div>
